...
class InformixReaderThread implements Runnable {
	...
	final static int MAX_SAMPLE_ROWS = 25;

	String generateTestData(TableState tblState) {
		CDC_Table_Schema schema = tblState.cdcTableSchema;
		int numCols = schema.columns.size();
		String outputString = "";
		String tableName = tblState.tableToMonitor.substring(tblState.tableToMonitor.lastIndexOf('.') + 1);
		int startValue = (int) Math.round(Math.random() * 255.0);

		for (int row = 0; row < m_SharedInfo.m_SampleRows; row++) {
			outputString += "insert into " + tableName + " values (";
			for (int col = 0; col < numCols; col++) {
				String strColumn = "";
				ColumnDef column = schema.columns.get(col);
				switch (column.ifxDataType) {
				case INTEGER:
					strColumn = String.format("%d", startValue + row + col);
					break;
				case CHARACTER: {
					int value = startValue + row + col;
					if (column.size < 8) {
						value &= (1 << (4 * column.size)) - 1;
					}
					strColumn = String.format("%x:", value);
					while (strColumn.length() < column.size) {
						strColumn += column.name + "+";
					}
					strColumn = "'" + strColumn.substring(0, column.size) + "'";
				}
					break;
				case VARCHAR: {
					int size = (int) Math.round(Math.random() * 255.0);
					int value = startValue + row + col;
					value &= (1 << (4 * size)) - 1;
					strColumn = String.format("%x:", value);
					while (strColumn.length() < size) {
						strColumn += column.name + "+";
					}
					strColumn = "'" + strColumn.substring(0, size) + "'";
				}
					break;
				default:
					break;
	...
	void cdcClose() throws IOException {
		...
		try {
			try {
				...
				m_SharedInfo.m_EventQueue.clear();
			} catch (Exception ex) {
				LOG.error("EventQueue", ex);
				ex.printStackTrace();
			}

			for (int idx = 0; idx < tableStates.size(); idx++) {
				TableState tblState = tableStates.get(idx);
				if (tblState == null) {
					continue;
				}
			...
			if (m_SharedInfo.m_SessionId >= 0) {
				closesess(m_SharedInfo.m_SessionId);
				m_SharedInfo.m_SessionId = 0;
			}
			if (m_SharedInfo.m_DBConn != null) {
				if (!m_SharedInfo.m_DBConn.isClosed()) {
					m_SharedInfo.m_DBConn.close();
	...
	int processRecCommitTX(DataInputStream dis, int bytesProcessed, String outStr) throws Exception {
		...
		LOG.info("CommitTX: SENT " + size + " operations");
	...
	int processRecIUD(DataInputStream dis, int bytesProcessed, int payloadSize, int recordNumber, String outStr)
		...
		bytesProcessed += payloadSize;
		{
			...
			LOG.info(tblState.tableToMonitor + ":columns <" + Arrays.toString(byteMessage.array()) + ">");
		}
	...
	int processRecTabSchema(DataInputStream dis, int bytesProcessed, int payloadSize, String outStr) throws Exception {
		...
		byte[] restOfPayload = new byte[payloadSize];
		dis.readFully(restOfPayload, 0, payloadSize);
		LOG.info("Columns: (" + new String(restOfPayload) + ")");
		...
		m_SharedInfo.m_EventQueue.add(ByteBuffer.wrap(generateTestData(tblState).getBytes()));

		return bytesProcessed;
	}

	/**
	 * Simulate the CDC C API call 'rtypsize' to return the number of bytes used
	 * by the column within the CDC record.
	 * 
	 * @param colDef
	 * @return
	 */
	...
	public void connect(String host, int port, String database, String iServer, String user, String pswd)
		...
		LOG.info("Loading jdbc driver to connect via url: " + connStr);
